			...
			while ($reg=$rspta->fetch_object()){
				$data[]=array(
					"0"=>($reg->condicion)?'<button class="btn btn-warning" onclick="mostrar('.$reg->idusuario.')"><i class="fa fa-pencil"></i></button>'.
						...
						' <button class="btn btn-warning" onclick="activar('.$reg->idusuario.')"><i class="fa fa-check"></i></button>',
					"1"=>$reg->nombre,
		...
		break;
				...
				case 'verificar':
					...
					if (isset($fetch))
				    {
						...
						in_array(1,$valores)?$_SESSION['inventarios']=1:$_SESSION['inventarios']=0;
