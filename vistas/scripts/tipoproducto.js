...
function listartipoproductos() {
  ...
  "ajax":
      {
        url: '../ajax/tipoproducto.php?op=listar',
        type : "get",
        dataType : "json",
        error: function(e){
...
function mostrar(idtipoproducto) {
  ...
  $.post("../ajax/tipoproducto.php?op=mostrar", {
    idtipoproducto: idtipoproducto
  }, function(data, status) {
    data = JSON.parse(data);

        var nom_mar = data.nombre;
        var idmar = data.idtipoproducto;
        var desc = data.descripcion;

    $.confirm({
        backgroundDismiss: true,
        title: 'Actualiza una tipoproducto',
        type: 'personal',
        content: '' +
        '<form name="formulario" id="formulario" method="POST">'+
        '<div class="form-group">' +
        '<label>Nombre Modulo</label>' +
        '<input type="hidden" name="idtipoproducto" id="idtipoproducto" value="'+idmar+'"/>' +
        '<input type="text" name="nombre" id="nombre" value="'+nom_mar+'" placeholder="Ingresa tipoproducto" class="name form-control" required />' +
        '</div>' +
        '<div class="form-group">' +
...
function editreg_form(){
  ...
  $.confirm({
      ...
      '<input type="text" name="nombre" id="nombre" placeholder="Ingresa tipo producto" class="name form-control" autofocus required />' +
      '</div>' +
      '<div class="form-group">' +
      '<label>Descripcion</label>' +
...
function desactivar(idtipoproducto) {
  ...
  $.confirm({
    ...
    icon: 'fa fa-warning',
    title: 'Advertencia!',
    content: '¿Está seguro de desactivar el tipo de producto Actual?',
    type: 'red',
   typeAnimated: true,
    buttons: {
        somethingElse: {
            ...
            keys: ['enter', 'shift'],
            action: function(){
                $.post("../ajax/tipoproducto.php?op=desactivar", {
                  idtipoproducto: idtipoproducto
                }, function(e) {
...
function activar(idtipoproducto) {
  $.confirm({
    ...
    title: 'Advertencia!',
    content: '¿Está seguro de activar el tipo de producto Actual?',
    type: 'yellow',
   typeAnimated: true,
    buttons: {
        somethingElse: {
            ...
            keys: ['enter', 'shift'],
            action: function(){
                $.post("../ajax/tipoproducto.php?op=activar", {
                  idtipoproducto: idtipoproducto
                }, function(e) {
...
function eliminar(idtipoproducto) {
  $.confirm({
    icon: 'fa fa-times',
    title: 'Peligro!',
    content: '¿Está seguro de eliminar el tipo de producto Actual?',
    type: 'red',
   typeAnimated: true,
    buttons: {
        somethingElse: {
            ...
            keys: ['enter', 'shift'],
            action: function(){
                $.post("../ajax/tipoproducto.php?op=eliminar", {
                  idtipoproducto: idtipoproducto
                }, function(e) {
